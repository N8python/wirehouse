export function createGameConstants({ THREE, cellSize, playerSpeed }) {
  ...
  const FOUND_FOOTAGE_GRAIN_INTENSITY = 0.075;
  const FOUND_FOOTAGE_GRAIN_STATIC_STRENGTH = 0.1;
  const PLAYER_SPRINT_FOV_BOOST_DEGREES = 10;
  const PLAYER_SPRINT_FOV_EMA_HALF_LIFE_SECONDS = 0.09;
  ...
  const PLAYER_SPRINT_DURATION_SECONDS = 7;
  const PLAYER_SPRINT_STAMINA_DRAIN_PER_SECOND = PLAYER_MAX_STAMINA / PLAYER_SPRINT_DURATION_SECONDS;
  const PLAYER_STAMINA_REGEN_DELAY_SECONDS = 3;
  const PLAYER_STAMINA_REGEN_PER_SECOND = PLAYER_MAX_STAMINA / PLAYER_SPRINT_DURATION_SECONDS;
  const STAMINA_LOW_HEARTBEAT_THRESHOLD_RATIO = 0.2;
  const STAMINA_LOW_SPRINT_HEARTBEAT_FREQUENCY_MULTIPLIER = 2.8;
  const WIREMAN_MODEL_PATH = "./assets/models/wireman.glb";
  const WIREMAN_WALK_SPEED = playerSpeed * 0.75;
  const WIREMAN_SPRINT_MULTIPLIER = 1.5;
  const WIREMAN_REPATH_INTERVAL_SECONDS = 0.3;
  ...
  return {
    ...
    PLAYER_MAX_HEALTH,
    PLAYER_MAX_STAMINA,
    PLAYER_SPRINT_DURATION_SECONDS,
